...
loss = tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits_v2(labels=Y, logits=Y_hat))
opt = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss)
saver = tf.train.Saver()
# tf.reset_default_graph()
imported_graph = tf.train.import_meta_graph(
    './trained_model/training-2800.meta')

with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    imported_graph.restore(sess, tf.train.latest_checkpoint('./trained_model'))
    ...
    for i in range(30000):
        for j in range(num_batches):
            ...
            pred, cost, _ = sess.run([Y_hat, loss, opt], feed_dict={
                                     X: X_batch, Y: Y_batch})
        ...
        if i % 250 == 0:
            pred_train_dev, cost, _ = sess.run([Y_hat, loss, opt], feed_dict={
                                               X: X_train_dev, Y: Y_train_dev_E})
    ...
    pred_test, cost, _ = sess.run([Y_hat, loss, opt], feed_dict={
                                  X: X_test, Y: Y_test_E})
